...
testLogMessage = do
    ...
    let m :: LogObject Text = LogObject (loggerNameFromText "test") meta (LogMessage "hello")
...
testLogValue = do
    meta <- mkLOMeta Info Public
    let m :: LogObject Text = LogObject (loggerNameFromText "test") meta (LogValue "value" (PureI 42))
    let encoded = encode m
    let decoded = decode encoded :: Maybe (LogObject Text)
    assertEqual "unequal" (Just m) decoded
...
testLogError = do
    meta <- mkLOMeta Info Public
    let m :: LogObject Text = LogObject (loggerNameFromText "test") meta (LogError "error")
    let encoded = encode m
    let decoded = decode encoded :: Maybe (LogObject Text)
    assertEqual "unequal" (Just m) decoded
...
testLogStructured = do
    meta <- mkLOMeta Info Public
    let m :: LogObject Text = LogObject (loggerNameFromText "test") meta . LogStructured $
...
testObserveOpen = do
    ...
    let m :: LogObject Text = LogObject (loggerNameFromText "test") meta (ObserveOpen cs)
    let encoded = encode m
    let decoded = decode encoded :: Maybe (LogObject Text)
    assertEqual "unequal" (Just m) decoded
...
testObserveDiff = do
    ...
    let cs = CounterState [Counter StatInfo "some" (Bytes 789),
                           Counter RTSStats "gcn" (PureI 42)]
    let m :: LogObject Text = LogObject (loggerNameFromText "test") meta (ObserveDiff cs)
    let encoded = encode m
    let decoded = decode encoded :: Maybe (LogObject Text)
    assertEqual "unequal" (Just m) decoded
...
testObserveClose = do
    ...
    let cs = CounterState [Counter StatInfo "some" (Bytes 789),
                           Counter RTSStats "gcn" (PureI 42)]
    let m :: LogObject Text = LogObject (loggerNameFromText "test") meta (ObserveClose cs)
    let encoded = encode m
    let decoded = decode encoded :: Maybe (LogObject Text)
    assertEqual "unequal" (Just m) decoded
...
testAggregatedMessage = do
    ...
    let m :: LogObject Text = LogObject (loggerNameFromText "test") meta (AggregatedMessage as)
    let encoded = encode m
    let decoded = decode encoded :: Maybe (LogObject Text)
    assertEqual "unequal" (Just m) decoded
...
testMonitoringEffect = do
    meta <- mkLOMeta Info Public
    let m :: LogObject Text = LogObject (loggerNameFromText "test")meta (MonitoringEffect (MonitorAlterGlobalSeverity Notice))
    let encoded = encode m
    let decoded = decode encoded :: Maybe (LogObject Text)
    assertEqual "unequal" (Just m) decoded
...
testCommand = do
    meta <- mkLOMeta Info Public
    let m :: LogObject Text = LogObject (loggerNameFromText "test")meta (Command (DumpBufferedTo KatipBK))
    let encoded = encode m
    let decoded = decode encoded :: Maybe (LogObject Text)
    assertEqual "unequal" (Just m) decoded
...
testKillPill = do
    meta <- mkLOMeta Info Public
    let m :: LogObject Text = LogObject (loggerNameFromText "test")meta KillPill
    let encoded = encode m
    let decoded = decode encoded :: Maybe (LogObject Text)
    assertEqual "unequal" (Just m) decoded
